...
plt.figure(figsize=(10, 6))
sns.lineplot(x='Year', y='Hours per Week', data = CanadianWorkHours)
plt.xlabel('Year')
...
Z_series_one = diff(CanadianWorkHours['Hours per Week'], k_diff = 1)
```
...
plt.figure(figsize=(10, 6))
sns.lineplot(x=CanadianWorkHours['Year'], y=Z_series_one)
plt.xlabel('Year')
...
Z_series_two = diff(CanadianWorkHours['Hours per Week'], k_diff = 2)
```
...
sns.lineplot(x=CanadianWorkHours['Year'], y=Z_series_two)
plt.xlabel('Year')
...
plt.figure(figsize=(3, 3))
plot_pacf(Z_series_two, lags = 15)
plt.xlabel("Lag")
plt.ylabel("Correlation")
plt.show()
```
...
Following part 1 of Rule 1, we conclude that an autoregressive term of order 1 will be sufficient to capture the relationships between the elements of the series.
...
Instead of using past values of the response variable, a moving average model uses stochastic terms to predict the current response.
The model has different versions depending on the number of errors used to predict the response. For example:
