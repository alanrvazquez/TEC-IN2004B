...
To determine if the RMSE is high or small, we must compare it with a *naive* forecast that predicts the response $Y_t$ with the previous one $Y_{t-1}$. To build this forecast, we follow two steps. 
...
We now input 35.9 as the first record in `naive_forecast`. The first record of the object is indexed by a 28.
...
naive_rmse = mean_squared_error(Canadian_validation["Hours per Week"], naive_forecast)
